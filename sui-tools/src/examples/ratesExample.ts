import { 
  ...
  getLendingRate, 
  aprToApy, 
  getBestLendingOpportunities,
  calculatePoolImpermanentLoss 
} from '../rates/rates';
...
async function main() {
  try {
    ...
    console.log('\nFetching pool APY...');
    try {
      const apy = await getPoolApy(poolId);
      if (apy === 0) {
        console.log('Note: Pool APY is currently 0%, calculating based on utilization...');
        const lendingRate = await getLendingRate(poolId);
    ...
    const calculatedApy = aprToApy(sampleApr / 100);
    console.log(`APR: ${sampleApr}% → APY: ${calculatedApy.toFixed(2)}%`);

    // Get best lending opportunities
    console.log('\nFetching best lending opportunities...');
    const bestOpportunities = await getBestLendingOpportunities(0); // Set minimum APY to 0 to see all pools
    if (bestOpportunities.length === 0) {
      ...
      bestOpportunities.forEach((pool: PoolInfo) => {
    ...
    examples.forEach(({ price_change, initial, current }) => {
      ...
      const impermanentLoss = calculatePoolImpermanentLoss(
        initial,
        current,
        poolShare
      );
